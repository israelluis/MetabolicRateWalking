function [Results,Misc,DatStore,lMo_scaling_param_opt,lTs_scaling_param_opt,kT_scaling_param_opt,EMGscale_opt] = ...
    runParameterEstimation2(Misc,DatStore,Mesh,SolverSetup,Results,NMuscles,MTlabel)
% --------------------------------------------------------------------------
...
if Misc.boolEMG
    ...
    % added coupling of EMG scaling
for k = 1:size(Misc.coupled_EMGscale,1) % israel
    for j = 1:size(Misc.coupled_EMGscale,2)
        Inds = find(strcmp(scaledEMGmusc,Misc.Coupled_EMG_scaling{k,j}));
        if ~isempty(Inds)
            Misc.coupled_EMGscale(k,j) = Inds;
        end
    end
end

% added EMG scaling coupling - israel
for k = 1:size(Misc.coupled_EMGscale,1)
    opti_MTE.subject_to(EMGscale(Misc.coupled_EMGscale(k,1)) - EMGscale(Misc.coupled_EMGscale(k,2)) == 0);
end
end
...
for trial = 1:Misc.nTrials
    ...
    % Set initial guess
    variableno=0;
    if Misc.MRSBool == 1 && variableno==0
...
%         Get muscle-tendon forces and derive Hill-equilibrium with passive parameters
        [Hilldiffk,FTk] = ForceEquilibrium_lMtildeState_lMoFree_lTsFree_kTFree_optPassive(ak,lMtildek,...
            vMtildek,lM_projectedk,DatStore(trial).LMTinterp(k,:)',...
            [lMo_scaling_param(Misc.idx_allMuscleList{trial},1) lTs_scaling_param(Misc.idx_allMuscleList{trial},1) kT_scaling_param(Misc.idx_allMuscleList{trial},1)],...
            Misc.params(:,Misc.idx_allMuscleList{trial})',Misc.kT(1,Misc.idx_allMuscleList{trial})',...
            Misc.kpe(1,Misc.idx_allMuscleList{trial})',... % kpe
            Misc.ksf(1,Misc.idx_allMuscleList{trial})');   % ksf            
...
diary(fullfile(Misc.OutPath,[Misc.OutName '_' MTlabel '.txt']));
tic
...
for trial = 1:Misc.nTrials
    ...
    % Save results
    Results.Time(trial).([MTlabel]) = tgrid;
    Results.MActivation(trial).([MTlabel]) = a_opt{trial};
    Results.lMtildeopt(trial).([MTlabel]) = lMtilde_opt{trial};
    Results.lM(trial).([MTlabel]) = lMtilde_opt{trial}.*repmat(lMo_opt(Misc.idx_allMuscleList{trial},1),1,length(tgrid));
    Results.vMtilde(trial).([MTlabel]) = vMtilde_opt{trial};
    Results.MExcitation(trial).([MTlabel]) = e_opt{trial};
    Results.RActivation(trial).([MTlabel]) = aT_opt{trial}*Misc.Topt;
    Results.lM_projected_opt(trial).([MTlabel]) = lM_projected_opt{trial};
    
    Results.lMTinterp(trial).([MTlabel]) = DatStore(trial).LMTinterp';
    % get force from muscle state
    [TForcetilde,TForce] = TendonForce_lMtilde(Results.lMtildeopt(trial).([MTlabel])',paramsOpt(:,Misc.idx_allMuscleList{trial}),Results.lMTinterp(trial).([MTlabel])',kTOpt(1,Misc.idx_allMuscleList{trial}),shiftOpt(1,Misc.idx_allMuscleList{trial}));
    Results.TForcetilde(trial).([MTlabel]) = TForcetilde';
    Results.TForce(trial).([MTlabel]) = TForce';
%     % vMtildeMax muscle property is not optimized
%     [Fpe,FMltilde,FMvtilde] = getForceLengthVelocityProperties(Results.lMtildeopt(trial).([MTlabel])',Results.vMtilde(trial).([MTlabel])',Misc.params(5,Misc.idx_allMuscleList{trial}));
    % vMtildeMax muscle property is not optimized
    [Fpe,FMltilde,FMvtilde] = getForceLengthVelocityProperties_setPassiveParam(Results.lMtildeopt(trial).([MTlabel])',Results.vMtilde(trial).([MTlabel])',Misc.params(5,Misc.idx_allMuscleList{trial}),...
    Misc.kpe(1,Misc.idx_allMuscleList{trial}),... % kpe input=varVal x muscle
    Misc.ksf(1,Misc.idx_allMuscleList{trial}));   % ksf input=varVal x muscle

    FMo = ones(N+1,1)*Misc.params(1,Misc.idx_allMuscleList{trial});
    Results.Fpe(trial).([MTlabel]) = (Fpe.*FMo)';
    Results.FMltilde(trial).([MTlabel]) = FMltilde';
    Results.FMvtilde(trial).([MTlabel]) = FMvtilde';
end
