...
%% parameters of passive curve
if ~isfield(Misc,'PassiveParam_kpe') || ~isfield(Misc,'PassiveParam_kpe')
    Misc.passiveCurveCal=[];
else
    Misc.passiveCurveCal=1;
end

if  isempty(Misc.passiveCurveCal)
    import org.opensim.modeling.*
    model     = Model(Misc.model_path);
    NumbMuscle=model.getMuscles().getSize;
    Misc.PassiveParam_kpe=ones(1,NumbMuscle)*4; % default value
    Misc.PassiveParam_ksf=ones(1,NumbMuscle)*1; % default value
    Misc.PassiveParam_ke0=ones(1,NumbMuscle)*0.6; % default value
end

if ~isfield(Misc,'use_filter') || ~isfield(Misc,'use_filter')
    Misc.use_filter =1;
end
if ~isfield(Misc,'use_filter_ID') || ~isfield(Misc,'use_filter_ID')
    Misc.use_filter_ID =0;
end
if ~isfield(Misc,'Coupled_EMG_scaling') || ~isfield(Misc,'Coupled_EMG_scaling')
    Misc.Coupled_EMG_scaling =[];
end
