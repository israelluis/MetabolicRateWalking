function [h] = PlotEMGTracking2(Results,DatStore,Misc)
...
for i=1:Misc.nTrials
    ...
    tSim_G = Results.Time(i).MTG;
    EMG_G = ppval(DatStore(i).EMG.EMGspline,tSim_G)';
    legend_title = {};
    for j=1:nEMG
        subplot(p(1),p(2),j);
        ...
        eSim_G = Results.MExcitation(i).MTG(EMGinds(j),:)'; %israel
        tSim_G = Results.Time(i).MTG;
        plot(tSim_G(1:end-1),eSim_G,'Color',Cs(1,:),'LineWidth',lw); hold on;
        
        if j == 1
            legend_title = [legend_title,'Parameter Estimation'];
        end
        % measured EMG
        EMGscaled = EMG(:,j).*Results.Param.EMGscale.MTE(DatStore(i).EMG.idx_EMGsel(j,4));
        plot(tSim,EMGscaled,'--k','LineWidth',lw);
        
        EMGscaled_G = EMG_G(:,j).*Results.Param.EMGscale.MTG(DatStore(i).EMG.idx_EMGsel(j,4)); % israel
        plot(tSim_G,EMGscaled_G,'--k','LineWidth',lw);        
