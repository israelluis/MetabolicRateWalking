function [Results,DatStore,Misc] = solveMuscleRedundancy2(time,Misc)
% -----------------------------------------------------------------------%
% INPUTS:
%           time: time window
...
%     Misc.BoolParamOpt = 1;
% end
if Misc.BoolParamGen==1
    Estimate_OptimalFiberLength_temp=Misc.Estimate_OptimalFiberLength;
    Estimate_TendonSlackLength_temp =Misc.Estimate_TendonSlackLength;
    Estimate_TendonStiffness_temp   =Misc.Estimate_TendonStiffness;
    Coupled_fiber_length_temp       =Misc.Coupled_fiber_length;
    Coupled_slack_length_temp       =Misc.Coupled_slack_length;
    Coupled_TendonStiffness_temp    =Misc.Coupled_TendonStiffness;
    coupled_lMo_temp                =Misc.coupled_lMo;
    coupled_lTs_temp                =Misc.coupled_lTs;
    coupled_kT_temp                 =Misc.coupled_kT;
    ...
    [Results,Misc,DatStore,~,~,...
        ~,EMGscale_gen] = runParameterEstimation2(Misc,...
        DatStore,Mesh,SolverSetup,Results,NMuscles,'MTG');  
    Misc.Estimate_OptimalFiberLength=Estimate_OptimalFiberLength_temp;
    Misc.Estimate_TendonSlackLength =Estimate_TendonSlackLength_temp;
    Misc.Estimate_TendonStiffness   =Estimate_TendonStiffness_temp;
    Misc.Coupled_fiber_length       =Coupled_fiber_length_temp;
    Misc.Coupled_slack_length       =Coupled_slack_length_temp;
    Misc.Coupled_TendonStiffness    =Coupled_TendonStiffness_temp;
    Misc.coupled_lMo                =coupled_lMo_temp;
    Misc.coupled_lTs                =coupled_lTs_temp;
    Misc.coupled_kT                 =coupled_kT_temp;
end
if Misc.BoolParamOpt == 1
    % Run the parameter estimation
    [Results,Misc,DatStore,lMo_scaling_param_opt,lTs_scaling_param_opt,...
        kT_scaling_param_opt,EMGscale_opt] = runParameterEstimation2(Misc,...
        DatStore,Mesh,SolverSetup,Results,NMuscles,'MTE');
else
...
if Misc.BoolParamOpt
    ...
    if Misc.boolEMG
        Results.Param.EMGscale.MTE = EMGscale_opt;
    end
end
if Misc.BoolParamGen
    Results.Param.EMGscale.MTG     = EMGscale_gen;
end
...
if Misc.PlotBool && Misc.EMGconstr == 1
    h = PlotEMGTracking2(Results,DatStore,Misc);
