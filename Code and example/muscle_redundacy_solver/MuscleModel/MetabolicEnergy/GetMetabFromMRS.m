...
for n = 1:length(Names)
    for tr = 1:Misc.nTrials
        ...
        for i = iSel
            [E_Bargh(:,i),E_Bargh_a(:,i),E_Bargh_m(:,i),E_Bargh_s(:,i),W_Bargh(:,i),E_Barh_model(i)] = ...
                getMetabolicEnergySmooth2004all(exc(:,i),act(:,i),lMtilde(:,i),vM(:,i),Fce(:,i),Fpass(:,i),...
                musclemass,pctst,FL_mult(:,i),Fiso,modelmass,b);
        end
        E_Bargh_am=E_Bargh_a+E_Bargh_m;
        
        % Umberger 2003
        E_Umb2003 = nan(size(act));
        E_Umb2003_model = nan(1,size(act,2));
        for i = iSel
            [E_Umb2003(:,i),E_Umb2003_am(:,i),E_Umb2003_sl(:,i),W_Umb2003(:,i),E_Umb2003_model(i)] = ...
        ...
        for i = iSel
            [E_Umb2010(:,i),E_Umb2010_am(:,i),E_Umb2010_sl(:,i),W_Umb2010(:,i),E_Umb2010_model(i)] = ...
        ...
        for i = iSel
            [E_Uch2016(:,i),E_Uch2016_am(:,i),E_Uch2016_sl(:,i),W_Uch2016(:,i),E_Uch2016_model(i)] = ...
        ...
        Energy(tr).(Names{n}).Bargh2004.E_am = E_Bargh_am;
        Energy(tr).(Names{n}).Bargh2004.E_sl = E_Bargh_s;
        Energy(tr).(Names{n}).Bargh2004.Wdot = W_Bargh;
        Energy(tr).(Names{n}).Bargh2004.Edot_model= E_Barh_model;
        
        % store metabolic power in a structure
        Energy(tr).(Names{n}).Umb2003.Edot = E_Umb2003;
        Energy(tr).(Names{n}).Umb2003.E_am = E_Umb2003_am;
        Energy(tr).(Names{n}).Umb2003.E_sl = E_Umb2003_sl;
        Energy(tr).(Names{n}).Umb2003.Wdot = W_Umb2003;
        Energy(tr).(Names{n}).Umb2003.Edot_model= E_Umb2003_model;
        
        % store metabolic power in a structure
        Energy(tr).(Names{n}).Umb2010.Edot = E_Umb2010;
        Energy(tr).(Names{n}).Umb2010.E_am = E_Umb2010_am;
        Energy(tr).(Names{n}).Umb2010.E_sl = E_Umb2010_sl;        
        Energy(tr).(Names{n}).Umb2010.Wdot = W_Umb2010;
        Energy(tr).(Names{n}).Umb2010.Edot_model= E_Umb2010_model;
        
        % store metabolic power in a structure
        Energy(tr).(Names{n}).Uch2016.Edot = E_Uch2016;
        Energy(tr).(Names{n}).Uch2016.E_am = E_Uch2016_am;
        Energy(tr).(Names{n}).Uch2016.E_sl = E_Uch2016_sl;         
        Energy(tr).(Names{n}).Uch2016.Wdot = W_Uch2016; 
